...
def PS2_Initial():
    ...
    ps2 = Robot_PS2(PS2_DAT_PIN, PS2_CMD_PIN, PS2_SEL_PIN, PS2_CLK_PIN)
...
def PS2_Control():
    ...
    #按键SELECT 来选择 MOVE_TAG 的模式
    if ps2.ButtonPressed('SELECT'):
        MOVE_TAG += 1
        if MOVE_TAG >=4:
            MOVE_TAG = 0
        print("MOVE_TAG = ",MOVE_TAG)
        
    #按键SELECT 来选择 PS2_CON_TAG 的模式
    elif ps2.ButtonPressed('START'):
        if MOVE_TAG == 0:
            print("START+tingzhi")
            Robot_CAR.tingzhi()
    ...
    elif ps2.ButtonPressed('PAD_UP') or ps2.ButtonPressed('TRIANGLE'):
        Robot_CAR.qianjin(SPEED_PWM_PS2)
    
    elif ps2.ButtonPressed('PAD_DOWN') or ps2.ButtonPressed('CROSS'):
        Robot_CAR.houtui(SPEED_PWM_PS2)
            
    elif ps2.ButtonPressed('PAD_LEFT') or ps2.ButtonPressed('SQUARE'):
        Robot_CAR.zuozhuan(SPEED_PWM_PS2)
            
    elif ps2.ButtonPressed('PAD_RIGHT') or ps2.ButtonPressed('CIRCLE'):
        Robot_CAR.youzhuan(SPEED_PWM_PS2)
            
    elif ps2.ButtonPressed('L1') :
        Robot_CAR.zuopingyi(SPEED_PWM_PS2)
        MOVE_TAG = 0
        print("MOVE_TAG = ",MOVE_TAG)
    elif ps2.ButtonPressed('R1') :
        Robot_CAR.youpingyi(SPEED_PWM_PS2)
        MOVE_TAG = 0
        print("MOVE_TAG = ",MOVE_TAG)
    
    elif ps2.ButtonPressed('L2') :
        Robot_CAR.tingzhi()
        
    elif ps2.ButtonPressed('R2') :
        Robot_CAR.tingzhi()
        
    elif ps2.ButtonReleased('PAD_UP') or ps2.ButtonReleased('PAD_DOWN') or ps2.ButtonReleased('PAD_LEFT') or ps2.ButtonReleased('PAD_RIGHT') or ps2.ButtonReleased('L1') or ps2.ButtonReleased('R1') or ps2.ButtonReleased('TRIANGLE') or ps2.ButtonReleased('CROSS') or ps2.ButtonReleased('SQUARE') or ps2.ButtonReleased('CIRCLE') :
        Robot_CAR.tingzhi()
    
    #处理手柄的遥感，根据遥感数值来计算车子的运动速度
    #此处的算法思路是，左右遥感模拟量先判断大小，取运动量小的。遥感中间是128，向上是0，向下是255
#     joysticks_left_x,joysticks_left_y,joysticks_right_x,joysticks_right_y = ps2.get_joysticks()
#     joysticks_right_x = ps2.Analog(5)
#     joysticks_right_y = ps2.Analog(6)
#     joysticks_left_x = ps2.Analog(7)
#     joysticks_left_y = ps2.Analog(8)
    #print(joysticks_left_x,joysticks_left_y,joysticks_right_x,joysticks_right_y)
    # 处理遥感数据，计算速度
    #当两个遥感都向下，车子倒退，取较小的数值计算速度
#     SPEED_PWM_PS2_1 = int(7.8 * (128 - joysticks_left_y))
#     SPEED_PWM_PS2_2 = int(7.8 * (128 - joysticks_right_y))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ps2.ButtonPressed('START') and ps2x.ButtonPressed('SELECT'):
        ...
        if ps2.en_Pressures:
            for button, label in zip([5, 6, 7, 8], ['Right X', 'Right Y', 'Left X', 'Left Y']):
                if ps2.Button('L1') or ps2.Button('R1'):
